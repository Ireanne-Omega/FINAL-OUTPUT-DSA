...
def additm():
    ...
    if item.get()!='':
        ...
        messagebox.showerror('Error','Please enter another one')


def gbill():
    if c_name.get() == "" or c_phone.get() == "":
        messagebox.showerror("Error", "right details are must")
    else:
...
def welcome():
    ...
    textarea.insert(END,"\n\t YHN CINEMA - AYLA MALLS")
    textarea.insert(END,f"\n\nTicket Reference Num:\t\t{bill_no.get()}")
    textarea.insert(END,f"\nCustomer Name:\t\t{c_name.get()}")
    textarea.insert(END,f"\nDate Today:\t\t{c_phone.get()}")
    textarea.insert(END,f"\n\n======================================")
    textarea.insert(END,"\n Ticket\t\tCinemaNum\t\tTime")
    textarea.insert(END,f"\n======================================\n")
...
cphone_lbl=Label(F1,text='Date ',font=('times new romon',18,'bold'),bg=bg_color,fg='white').grid(row=0,column=2,padx=20,pady=5)
...
F2 = LabelFrame(root, text='Movie Details', font=('times new romon', 18, 'bold'), fg='gold',bg=bg_color)
F2.place(x=20, y=180,width=630,height=500)

itm= Label(F2, text='Movie Name', font=('times new romon',18, 'bold'), bg=bg_color, fg='white').grid(
...
rate= Label(F2, text='Cinema Number', font=('times new romon',18, 'bold'), bg=bg_color, fg='white').grid(
...
n= Label(F2, text='Select a Time \n(between 1-5 PM only)', font=('times new romon',18, 'bold'), bg=bg_color, fg='white').grid(
...
bill_title=Label(F3,text='Ticket Receipt Section',font='arial 15 bold',bd=7,relief=GROOVE).pack(fill=X)
...
btn3=Button(F2,text='Delete',font='arial 15 bold',padx=5,pady=10,command=clear,bg='white',width=15)
btn3.grid(row=4,column=0,padx=10,pady=30)
btn4=Button(F2,text='Exit',font='arial 15 bold',padx=5,pady=10,command=exit,bg='gold',width=15)
